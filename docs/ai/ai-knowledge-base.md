...
export class ContextManager {
  ...
  estimateTokens() {
    // 累加所有消息的文字长度
    const totalLength = this.context.reduce(
 ...
 * 功能：基于【业务消息重要性评分】实现智能上下文压缩，自动保留关键业务对话，丢弃无关内容
...
class SmartContextManager {
   ...
   * 执行逻辑：
   * 1. 对所有历史消息进行业务重要性评分
   * 2. 按分数从高到低排序（业务价值越高越靠前）
   * 3. 根据当前Token占用量，智能计算需要保留的消息数量
   * 4. 保留高分业务消息，并按原始对话顺序恢复，保证业务逻辑连贯
   * 5. 用筛选后的高质量业务上下文替换原有上下文
   */
  compressContext() {
    // 获取当前全部对话上下文消息
    const messages = this.context

    // 1. 对每条消息执行【业务重要性评分】，返回分数数组
    const scores = this.scoreMessages(messages)
    
    // 2. 为每条消息绑定分数+原始索引，按业务重要性降序排列
    const sortedMessages = messages
      .map((msg, index) => ({ msg, score: scores[index], index }))
      .sort((a, b) => b.score - a.score)

    // 3. 根据Token使用情况，智能计算本轮应保留的消息条数
    const keepCount = this.calculateKeepCount()

    // 4. 提取分数最高的N条消息 → 按原始对话顺序排序 → 只保留消息内容
    const keepMessages = sortedMessages
      .slice(0, keepCount) 
      .sort((a, b) => a.index - b.index) 
      .map(item => item.msg)

    // 5. 更新上下文，只保留高价值业务消息
    this.context = keepMessages
  }

  /**
   * 【企业业务专属】消息重要性评分算法
   * 评分越高 = 业务价值越高 = 越不会被压缩删除
   * 完全根据企业业务场景定制：流程、需求、评审、制度、规范、政策等
   * @param {Array} messages - 对话消息列表
   * @returns {Array} 每条消息对应的业务重要性分数
   */
  scoreMessages(messages) {
    return messages.map((msg, index) => {
      let score = 0

      // 1. 【时效性加分】越新的消息，与当前业务问题相关性越强
      score += (messages.length - index) * 10

      // 2. 【用户提问加分】用户的业务问题是核心意图，必须优先保留
      if (msg.role === 'user') {
        score += 20
      }

      // 3. 【业务关键词加分 → 权重最高】
      // 覆盖：业务流程、需求、评审、制度、规范、项目、政策、变更、风险等企业核心内容
      const businessKeywords = [
        '需求', '评审', '流程', '规范', '制度', '业务', '项目',
        '重要', '关键', '必须', '注意', '政策', '变更', '风险',
        '上线', '版本', '规则', '问题', '方案', '组织', '职责'
      ]
      if (businessKeywords.some(kw => msg.content.includes(kw))) {
        score += 40  // 业务关键词权重最高，确保核心信息不丢失
      }

      // 4. 【取消长文本扣分】
      // 业务场景下：制度、流程、方案、评审纪要本身内容较长，长度≠冗余，不扣分
      // 原代码：if (msg.content.length > 500) score -= 10
  ...
  /**
   * 【动态保留策略】根据Token占用智能调整保留消息数量
   * 企业业务场景：Token充足则保留完整上下文；Token紧张则优先保留高价值业务消息
   * @returns {number} 最终需要保留的消息条数
   */
  calculateKeepCount() {
    // 估算当前上下文总Token
    const tokens = this.estimateTokens()

    // 情况1：Token使用 < 50% → 上下文充足，保留全部消息
    if (tokens < this.maxTokens * 0.5) {
      return this.context.length 
    }
    // 情况2：Token使用 50%~80% → 温和压缩，保留70%消息
    else if (tokens < this.maxTokens * 0.8) {
      return Math.ceil(this.context.length * 0.7)
    }
    // 情况3：Token使用 > 80% → 重度压缩，只保留50%最关键的业务消息
    else {
